"""test_session.py – Testes unitários básicos para VassouraSession."""
import pandas as pd
import numpy as np
import pytest
...
def test_corr_removal(df_toy):
    vs = VassouraSession(df_toy, target_col="d", heuristics=["corr"], thresholds={"corr": 0.85})
    df_clean = vs.run()
...
def test_vif_removal(df_toy):
    vs = VassouraSession(df_toy, target_col="d", heuristics=["vif"], thresholds={"vif": 5})
    df_clean = vs.run()
...
def test_iv_removal(df_toy):
    vs = VassouraSession(df_toy, target_col="d", heuristics=["iv"], thresholds={"iv": 0.0001})
    df_clean = vs.run()
    assert "iv<" in vs.history[-1]["reason"] or len(vs.dropped) == 0


def test_manual_removal(df_toy):
    vs = VassouraSession(df_toy, target_col="d")
